    ...
    public class DataAccessService : IDataAccessService
    {
        ...
        public async Task<List<NavigationMenuViewModel>> GetMenuItemsAsync(ClaimsPrincipal principal)
        {
            try
            {
                ...
                var res = await (from menu in _context.NavigationMenu select menu).ToListAsync();
        ...
        public async Task<bool> GetIdentityMenuItemsAsync(ClaimsPrincipal ctx, string PageName, string Area)
        {
            var result = false;
            var roleIds = await GetUserRoleIds(ctx);
            var data = await (from menu in _context.RoleMenuPermission
            ...
            foreach (var item in data)
            {
                result = (item.Area == Area && item.PageName == PageName);
                if (result)
                {
                    break;
                }
